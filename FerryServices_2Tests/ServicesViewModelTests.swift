...
final class ServicesViewModelTests: XCTestCase {
    func testGroupedSectionsIncludesSubscribedFirstAndSortedOperatorSections() {
        ...
        guard case .multiple(let sections) = viewModel.sections else {
            return XCTFail("Expected grouped sections")
        }

        XCTAssertEqual(sections.first?.title, "Subscribed")
        XCTAssertEqual(sections.first?.rows.map(\.service.serviceId), [2])

        let operatorSectionTitles = Array(sections.dropFirst().map(\.title))
        XCTAssertEqual(operatorSectionTitles, ["Alpha Ferries", "Beta Ferries"])
    }

    func testSearchTextFiltersCaseInsensitivelyAcrossAreaAndRoute() {
        let services = [
            TestDataFactory.makeService(
                id: 1,
                area: "Arran",
        ...
        guard case .single(let rows) = viewModel.sections else {
            return XCTFail("Expected single search section")
        }
        XCTAssertEqual(rows.map(\.service.serviceId), [1])

        viewModel.searchText = "armadale"
        guard case .single(let routeRows) = viewModel.sections else {
            return XCTFail("Expected single search section")
        }
        XCTAssertEqual(routeRows.map(\.service.serviceId), [2])
    }

    func testRowDisruptionTextMatchesStatus() {
        ...
        XCTAssertEqual(row.disruptionText, "Sailings Cancelled")
    }
}
